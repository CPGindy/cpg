            ...
            if rec.move_type == 'out_invoice':
                if rec.invoice_line_ids:
                    ...
                    for invoice_line in rec.invoice_line_ids.filtered(
                        ...
                        if len(invoice_line.product_id.license_product) > 0:
                            royaltable_amount = float(invoice_line.price_subtotal) / len(invoice_line.product_id.license_product)

                            for lic_prod in invoice_line.product_id.license_product:
                    ...
                    for invoice_line in rec.invoice_line_ids.filtered(
                        ...
                        for component in components:
                            ...
                            if any([license.license_item_id.license_status in ['active', 'revise', 'pending', 'inactive', 'discontinued'] for license in product.license_product]):
                                royaltable_components.append(component)
                            if not product.license_product:
                                non_licensed_components.append(component)

                        component_count = len(royaltable_components) + len(non_licensed_components)

                        #Create Zero royalties for the kit products who dont have license updated from decision tree on 12/16/2021
                        ...
                        if royaltable_components:
                            ...
                            for r_component in royaltable_components:
                                artwork_count = len(r_component.product_id.license_product)

                                for lic_prod in r_component.product_id.license_product:
                                    if lic_prod.license_item_id.license_type != 'flat':
                                        type = 'sale_on_item'
                                        royalty_rate = lic_prod.royalty_rate
