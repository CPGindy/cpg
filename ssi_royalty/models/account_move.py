    ...
    def _post(self, soft=True):
        ...
        for rec in self:
            if rec.invoice_line_ids:
                for invoice_line in rec.invoice_line_ids.filtered(lambda pro: pro.product_id.license_product and pro.move_id.move_type != 'in_invoice'):
                    ...
                    for lic_prod in invoice_line.product_id.license_product:
                        ...
                        for pool in lic_prod.license_item_id.item_pool_id.filtered(lambda p: not p.first_sale_date and p.status == 'paid'):
                            ...
                            search_pool_rec = self.env['ssi_royalty.pool'].search([('licensor_id', '=', royalty.licensor_id.id)])
                            if search_pool_rec:
                                line_vals = {
                                    'date': date.today(),
                            ...
                            elif royalty.licensor_id:
                               pool_val = {
                                   'licensor_id': royalty.licensor_id.id,
                                   'balance': 0.0
                               } 
