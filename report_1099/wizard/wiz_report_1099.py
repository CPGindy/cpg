            ...
            if self.report_option == 'eligible_vendors':
                ...
                if not (st_dt <= fiscal_en_dt and st_dt >= fiscal_st_dt) or \
                    ...
                    raise ValidationError(_(
                        " Start / End date should belongs to the same fiscal year !"))
    ...
    def get_invoices(self):
        ...
        if self.report_option == 'eligible_vendors':
            ...
            for payment_rec in payments_recs:
                ...
                if payment_rec.currency_id != usd_currency:
                    converted_amt = payment_rec.currency_id._convert(
                        ...
                        payment_rec.company_id, payment_rec.date
                    )
                    amt = converted_amt
            ...
            for pay_rec in payments_recs.filtered(lambda p: p.partner_id.id
                ...
                for payment_inv in pay_rec.reconciled_bill_ids:
        ...
        for inv in invoice_ids:
            ...
            if payment_dict_to_show:
                ...
                payment_dict.update({
                    ...
                    'invoice_state': inv.payment_state == 'paid' and 'paid' or 'open',
                })
                for inv_line in inv.invoice_line_ids.filtered(
                    ...
                    if inv.state != 'posted':
                        ...
                        debit_line_dict = {
                            ...
                            'currency_id': debit_line.currency_id and debit_line.currency_id.id or False
                        }
    ...
    def print_xls_report(self):
        ...
        for partner_id, income_dict in data_dict.items():
            ...
            worksheet.write_merge(1, 4, 6, 8,
                                  '1 Rents ' + '\n\n\n' + '$' + str(
                                      income_dict.get('rent', 0)), style_wrap)
            worksheet.write_merge(5, 8, 6, 8,
                                  '2 Royalties' + '\n\n\n' + '$' + str(
                                      income_dict.get('royalty', 0)),
                                  style_wrap)
            worksheet.write_merge(9, 12, 6, 8,
                                  '3 Other Income' + '\n\n\n' + '$' + str(
                                      income_dict.get('other', 0)),
                                  style_wrap)
            ...
            worksheet.write_merge(17, 20, 0, 5,
                                  'RECIPIENT’S name' + '\n    ' + str(
                                      partner_name), style_wrap)
            worksheet.write_merge(17, 20, 6, 8,
                                  '7 Nonemployee compensation' + '\n\n\n' + '$' + str(
                                      income_dict.get('non_emp_cmpr', 0)),
                                  style_wrap)
            ...
            worksheet.write_merge(
                ...
                '9 Payer made direct sales of \n   $5,000 or more of consumer \n   products to a buyer \n   (recipient) for resale ▶' + '\n\n' + '$' + str(
                    income_dict.get('sale', 0)), style_wrap)
            worksheet.write_merge(
                21, 24, 9, 11,
                '10 Crop insurance proceeds' + '\n\n\n' + '$' + str(
                    income_dict.get(
                        'crop_insurance_proceeds', 0)), style_wrap)
            ...
            worksheet.write_merge(
                ...
                '13 Excess golden parachute \n     payments' + '\n\n' + '$' + str(income_dict.get(
                    'excess_golden_perachute_payments', 0)), style_wrap)
            worksheet.write_merge(
                29, 32, 9, 11,
                '14 Gross proceeds paid to an attorney' + '\n\n\n' + '$' + str(
                    income_dict.get(
                        'gross_proceeds_paid_to_an_attomey', 0)), style_wrap)

            worksheet.write_merge(
                33, 36, 0, 2, '15a Section 409A deferrals' + '\n\n\n' + '$' + str(
                    income_dict.get('section_409A_deferrals', 0)), style_wrap)
            worksheet.write_merge(
                33, 36, 3, 5,
                '15b Section 409A income' + '\n\n\n' + '$' + str(
                    income_dict.get('section_409A_income', 0)), style_wrap)
            worksheet.write_merge(
                33, 36, 6, 8,
                '16 State tax withheld' + '\n\n\n' + '$' + str(
                    income_dict.get('state_tax_withheld', 0)), style_wrap)
            worksheet.write_merge(
                33, 36, 9, 11,
                '17 State/Payer’s state no.' + '\n\n\n' + '$' + str(
                    income_dict.get('state_payers_state_no', 0)), style_wrap)
            worksheet.write_merge(
                33, 36, 12, 14,
                '18 State income.' + '\n\n\n' + '$' + str(
                    income_dict.get('state_income', 0)), style_wrap)
